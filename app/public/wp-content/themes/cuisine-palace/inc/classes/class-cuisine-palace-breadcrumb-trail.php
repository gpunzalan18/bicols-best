 ...
 * @return string
 */
function cuisine_palace_breadcrumb_trail( $args = array() ) {
	$breadcrumb = new Cuisine_Palace_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class Cuisine_Palace_Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'cuisine_palace_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		$breadcrumb = apply_filters( 'cuisine_palace_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			...
			'browse'              => esc_html__( 'Browse:', 'cuisine-palace' ),

			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'cuisine-palace' ),

			'home'                => esc_html__( 'Home', 'cuisine-palace' ),

			'error_404'           => esc_html__( '404 Not Found', 'cuisine-palace' ),

			'archives'            => esc_html__( 'Archives', 'cuisine-palace' ),

			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'cuisine-palace' ),

			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'cuisine-palace' ),

			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'cuisine-palace' ),

			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'cuisine-palace' ),
		...
		$this->labels = apply_filters( 'cuisine_palace_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'cuisine_palace_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'cuisine_palace_breadcrumb_trail_items', $this->items, $this->args ) );
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'cuisine-palace' ) ) );
		}
	...
	protected function add_minute_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'cuisine-palace' ) ) );
		}
	...
	protected function add_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'cuisine-palace' ) ) );
		}
	...
	protected function add_day_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'cuisine-palace' ) ) );

		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'cuisine-palace' ) ) );

		$day = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'cuisine-palace' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'cuisine-palace' ) ) );
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					...
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'cuisine-palace' ) ) ) );
				} // If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {

					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'cuisine-palace' ) ) ) );
				} // If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {

					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'cuisine-palace' ) ) ) );
