...
class RPRESS_Batch_FoodItems_Import extends RPRESS_Batch_Import {
		...
		if( ! $this->done && $this->csv->data ) {
			...
			foreach( $this->csv->data as $key => $row ) {
				...
				if( ! empty( $this->field_mapping['categories'] ) && ! empty( $row[ $this->field_mapping['categories'] ] ) ) {
					...
					$this->set_taxonomy_terms( $fooditem_id, $categories, 'food-category' );
				...
				if( ! empty( $this->field_mapping['addons'] ) && ! empty( $row[ $this->field_mapping['addons'] ] ) ) {
					...
					$this->set_taxonomy_terms( $fooditem_id, $addons, 'addon_category' );
	 ...
	 * @return void
	 */
	private function set_taxonomy_terms( $fooditem_id = 0, $terms = array(), $taxonomy = 'food-category' ) {
	 ...
	 * @return array
	 */
	private function maybe_create_terms( $terms = array(), $taxonomy = 'food-category' ) {
