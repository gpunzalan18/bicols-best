...
class RestroPress_License {
	...
	public function notices() {
		...
		if( is_object( $license ) && 'valid' !== $license->license && empty( $showed_invalid_message ) ) {
			...
			if( empty( $_GET['page'] ) || 'rpress-extensions' !== $_GET['page'] ) {

				$messages[] = sprintf(
					__( 'You have invalid or expired license keys for RestroPress. Please go to the <a href="%s">Extensions page</a> to correct this issue.', 'restropress' ),
					admin_url( 'admin.php?page=rpress-extensions' )
				);
	...
	public function plugin_row_license_missing( $plugin_data, $version_info ) {
		...
		if( ( ! is_object( $license ) || 'valid' !== $license->license ) && empty( $showed_imissing_key_message[ $this->item_shortname ] ) ) {
			...
			echo '&nbsp;<strong><a href="' . esc_url( admin_url( 'admin.php?page=rpress-extensions' ) ) . '">' . __( 'Enter valid license key for automatic updates.', 'restropress' ) . '</a></strong>';
