...
jQuery( function($) {
  ...
  $( document ).on( 'click', '.submit-fooditem-button', function(e) {
    ...
    // var d = rp_getCookie('service_date');
    // d = new Date(d)
    // d.setDate(d.getDate()+1)
    // d = d.toLocaleString().split(",")[0]
    
    // 	rp_setCookie( 'service_date', d, rp_scripts.expire_cookie_time )
  ...
  $( 'body' ).on( 'click', '.rpress-delivery-opt-update', function( e ) {
    ...
    var serviceDate = _self.parents('.rpress-tabs-wrapper').find('.delivery-settings-wrapper.active .custom-pickup-date').val();
  ...
  $( document ).on( 'click', '.delivery-change', function(e) {
    ...
    var ServiceType  = rp_getCookie( 'service_type' );
    var ServiceTime  = rp_getCookie( 'service_time' );
	var DeliveryDate  = rp_getCookie( 'delivery_date' );
      ...
      success: function( response ) {
        ...
        if ( $( '.rpress-tabs-wrapper' ).length ) {
          ...
          if( ServiceTime !== '' ) {
			...
			$( '.custom-pickup-date').val(DeliveryDate);
          }
